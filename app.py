...
def load_and_process_data(uploaded_file):
    try:
        ...
        date_cols = [
            'Datum aanmaak','Startdatum intern','Einddatum intern',
...
def calculate_metrics(df):
    total = len(df)
    filled = df['Status vacature'].isin(FILLED_STATUSES).sum()
    open_ = total - filled
    not_filled = (df['Status vacature']=='Niet vervuld').sum()
    fill_rate = (filled/total*100) if total>0 else 0
    return {
        'total_vacatures': total,
        'vervulde_vacatures': filled,
        'openstaande_vacatures': open_,
        'niet_vervulde_vacatures': not_filled,
        'fill_rate': fill_rate
    }
...
# ---------------------
# Functie: Stacked bar per recruiter
# ---------------------
def create_recruiter_performance_chart(df):
    dfc = df[df['Eigenaar'].notna() & (df['Eigenaar'].str.strip()!='')]
    grp = dfc.groupby('Eigenaar').agg(Totaal=('Functie','count'))
    grp['Gevuld'] = dfc['Status vacature'].isin(FILLED_STATUSES).groupby(dfc['Eigenaar']).sum()
    grp['Open'] = grp['Totaal'] - grp['Gevuld']
    grp = grp.sort_values('Totaal')

    fig = go.Figure()
    fig.add_trace(go.Bar(
        y=grp.index, x=grp['Open'],
        name='Open', orientation='h'
    ))
    fig.add_trace(go.Bar(
        y=grp.index, x=grp['Gevuld'],
        name='Gevuld', orientation='h'
    ))
    fig.update_layout(
        barmode='stack',
        title="Aantal Vacatures per Recruiter (Open vs Gevuld)",
        xaxis_title="Aantal Vacatures",
        yaxis_title="Recruiter",
        height=600
    )
    return fig, grp
...
def main():
    ...
    # Tab 2: Recruiter performance + overzicht
    with tab2:
        st.header("Recruiter Performance")
        fig, rec_stats = create_recruiter_performance_chart(df)
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("Overzicht Openstaande Vacatures (globaal)")
        df_overview = df.copy()
        df_overview['Gevuld'] = df_overview['Status vacature'].isin(FILLED_STATUSES)
        # kanaalkolommen dynamisch selecteren
        channel_cols = [col for col in df_overview.columns if col.startswith('Totaal per wervingskanaal')]
        cols = ['Functie','Eigenaar','Datum aanmaak','Gevuld','Aantal reacties'] + channel_cols
        st.dataframe(df_overview[cols], use_container_width=True)
